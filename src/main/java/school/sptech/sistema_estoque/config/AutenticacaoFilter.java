...
import school.sptech.sistema_estoque.controller.AlmoxarifeController;
...
public class AutenticacaoFilter extends OncePerRequestFilter {
    ...
    // extrai o cookie da requisicao
    private String extrairToken(HttpServletRequest request) {
        // Prioridade 1: cookie HttpOnly (browser)
        Cookie[] cookies = request.getCookies();
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                if (AlmoxarifeController.COOKIE_NOME.equals(cookie.getName())) {
    ...
    // registra o token no contexto da thread
